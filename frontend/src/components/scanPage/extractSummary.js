export function extractSummary(scan) {
  ...
  const subdomains = Array.isArray(result.crtsh?.subdomains)
    ? result.crtsh.subdomains
    : [];
  const robots =
    typeof result.robots === "string"
      ? result.robots.trim()
      : result.robots || "";
  ...
  const serverSignature =
    ep?.details?.serverSignature || homeHeaders.server || "n/a";
  const protocolsArr = Array.isArray(ep?.details?.protocols)
    ? ep.details.protocols
    : [];
  const protocols = protocolsArr
  ...
  const suitesGroups = Array.isArray(ep?.details?.suites)
    ? ep.details.suites
    : [];
  const allSuites = suitesGroups.flatMap((g) =>
    Array.isArray(g?.list) ? g.list : []
  );
  const strongestCipher =
    allSuites
      .slice()
      .sort((a, b) => (b.cipherStrength ?? 0) - (a.cipherStrength ?? 0))[0]
      ?.name || null;
  ...
  const headerKeys = Object.keys(homeHeaders).reduce(
    (a, k) => a.add(k.toLowerCase()),
    new Set()
  );
  const hasCF = headerKeys.has("cf-ray") || headerKeys.has("cf-cache-status");
  const hasCloudFront =
    headerKeys.has("x-amz-cf-pop") || /cloudfront/i.test(serverSignature || "");
  const hasAkamai =
    headerKeys.has("server-timing") &&
    /ak_/.test(homeHeaders["server-timing"] || "");
  const cdnProvider = hasCF
    ? "Cloudflare"
    : hasCloudFront
    ? "AWS CloudFront"
    : hasAkamai
    ? "Akamai (heuristic)"
    : /google|gws/i.test(serverSignature || "")
    ? "Google Frontend (heuristic)"
    : /nginx|apache|iis/i.test(serverSignature || "")
    ? "Origin (nginx/apache/iis)"
    : "Unknown";
  ...
  const sitemap =
    robotsLines
      ...
      ?.replace(/^Sitemap:\s*/i, "") || null;
  const suspiciousDisallows = disallows.filter((d) =>
    /(admin|login|wp-admin|wp-login|phpmyadmin|backup|staging|test|old|private)/i.test(
      d
    )
  );
  ...
  if (
    !homeHeaders["strict-transport-security"] &&
    hstsPolicyStatus === "absent"
  ) {
    risks.push({
      id: "missing_hsts",
      severity: "medium",
  ...
  if (g && (["E", "F"].includes(g) || g < "B")) {
    risks.push({
      id: "weak_ssl",
      severity: "high",
      message: `SSL grade ${sslGrade} is weak`,
    });
  }
  if (certDaysLeft !== null && certDaysLeft < 75) {
    risks.push({
      id: "cert_expiry_soon",
      severity: "high",
      message: `Certificate expires in ${certDaysLeft} days`,
    });
  }
  if (weakCiphers.length) {
    risks.push({
      id: "weak_ciphers",
      severity: "medium",
      message: `Weak ciphers detected: ${weakCiphers.slice(0, 5).join(", ")}${
        weakCiphers.length > 5 ? "…" : ""
      }`,
    });
  }
  ...
  if (missingHeaders.includes("content-security-policy"))
    recommendations.push(
      "Add a strict Content-Security-Policy to reduce XSS risk."
    );
  if (!modernTLS)
    recommendations.push(
      "Enable TLS 1.3 and strong ciphers; disable legacy protocols."
    );
  if (weakCiphers.length)
    recommendations.push("Remove legacy ciphers (RC4/3DES/MD5/EXPORT).");
  if (certDaysLeft !== null && certDaysLeft < 75)
    recommendations.push("Renew the TLS certificate soon.");
  if (suspiciousDisallows.length)
    recommendations.push(
      "Review robots.txt disallows for sensitive admin paths exposure."
    );

  const urlscan = result.urlscan || null;
  const urlscanSummary =
    urlscan?.page?.title || urlscan?.data?.task?.domain || null;
  const urlscanScreenshot =
    urlscan?.screenshot || urlscan?.page?.screenshot || null;

  const techs = [];
  if (Array.isArray(result.wappalyzer?.applications)) {
    for (
      let i = 0;
      i < Math.min(6, result.wappalyzer.applications.length);
      i++
    ) {
    ...
    if (bw?.Result?.Technologies) {
      for (let i = 0; i < Math.min(6, bw.Result.Technologies.length); i++) {
        techs.push(
          bw.Result.Technologies[i].Name || bw.Result.Technologies[i].Tag
        );
      }
    }
  }
  ...
  const headline = [
    ...
    libs.length
      ? `Tech: ${libs.slice(0, 3).join(", ")}${libs.length > 3 ? "…" : ""}`
      : null,
