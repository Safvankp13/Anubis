...
const Dashboard = () => {
  ...
  const handleSaveAll = async () => {
    for (let sc of scans) {
      await saveReport({ scanId: sc._id, domain: sc.target });
    }
  ...
  const worstDomains = scans.slice()
    ...
    .map(s => ({
      ...
      meta: {
        subdomains: s.result.subdomainCount,
        tls13: s.result.modernTLS
      }
    }));
  ...
  return (
    <div className="flex">
      ...
      <div className="min-h-screen bg-[#0b0b0c] text-gray-100">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Passive scan insights across your monitored assets
              </p>
            </div>

            <div className="flex items-center gap-3">
              <button
                className="text-sm px-3 py-2 rounded-md bg-[#1a1b20] border border-[#2a2b31] hover:bg-[#202127]"
                type="button"
        ...
        <div className="px-6 pt-5 grid gap-4 grid-cols-1 sm:grid-cols-2 xl:grid-cols-4">
          {stats.map((s, i) => (
          ...
          <div className="col-span-12 xl:col-span-8">
            <div className="rounded-xl overflow-hidden border border-[#1f1f22] bg-[#111214]">
              ...
              <div className="grid grid-cols-12 bg-[#0d0e12] text-xs text-gray-400 border-b border-[#1f1f22]">
                <div className="col-span-5 px-4 py-2">Target</div>
                <div className="col-span-2 px-4 py-2">Grade</div>
                <div className="col-span-2 px-4 py-2">Worst Severity</div>
                <div className="col-span-1 px-4 py-2 text-right">Findings</div>
                <div className="col-span-2 px-4 py-2 text-center">Save</div>
                ...
                {recentScans.map((s, i) => (
                  <div key={i} className="grid grid-cols-12 items-center hover:bg-[#15161b] transition">
                    ...
                    <div className="col-span-2 px-4 py-3">
                      <span className={`text-xs px-2 py-[3px] rounded-md ${gradeBadge(s.result.sslGrade)}`}>
                        {s.result.sslGrade}
                      </span>
                    </div>

                    <div className="col-span-2 px-4 py-3">
                      <span className={`text-[11px] px-2 py-[3px] rounded-full font-semibold ${sevPill(s.result.worstSeverity)}`}>
                        {String(s.result.worstSeverity || "info").toUpperCase()}
                      </span>
                    </div>

                    <div className="col-span-1 px-4 py-3 text-right text-sm">
                    ...
                    <div className="col-span-2 px-4 py-3 text-center">
                      <button
                        type="button"
                        className="text-[11px] bg-[#03a48c] text-[#101213] px-2 py-1 rounded hover:bg-[#029e85]"
          ...
          <div className="col-span-12 xl:col-span-4">
            <div className="rounded-xl border border-[#1f1f22] bg-[#111214] overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="truncate">
                        ...
                        <div className="text-[11px] text-gray-500">
                          Subdomains: {w.meta.subdomains} • TLS1.3: {w.meta.tls13 ? "Yes" : "No"}
                        </div>
                      </div>
                      <span className={`text-[11px] px-2 py-[3px] rounded-full font-semibold ${sevPill(w.worst)}`}>
                        {w.worst.toUpperCase()}
                      </span>
                    </div>
                ...
                <div className="pt-1">
                  <button onClick={toggleExpand} className="text-xs text-gray-400 hover:text-gray-200" type="button">
                    { expandedDomain ? "Hide" : "View all →"}
                  </button>
                </div>
              </div>
            </div>
      ...
      {toast && (
  <div className="fixed top-10 right-30 bg-[#03a47c] text-white px-4 py-2 rounded-md shadow-lg text-sm animate-fade-in">
    {toast}
  </div>
)}
    </div>
